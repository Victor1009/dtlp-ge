...
function getDateFromAbreviation(date_part,value) {
    var serializedData = {};
    serializedData.option = "getDateFromAbreviation";
    serializedData.date_part = date_part;
    ...
    $.ajax({
        ...
        success: function (data) {
            array_values = ConvertStringToArray(data, "}", "]")
            DATE_TIME.request = {};
            DATE_TIME.request.start_date = array_values[0][0];
            DATE_TIME.request.end_date = array_values[0][1];            
        },
        error: function (jqXHR, textStatus, errorThrown) {
            alert(serializedData.option + ':' + textStatus + errorThrown);
        }
